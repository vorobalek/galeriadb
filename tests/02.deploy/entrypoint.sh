...
case "$CASE_ARG" in
  ...
  slow-sst) CASE_ARG="05.slow-sst-joiner" ;;
esac
...
if [ -n "$CASE_ARG" ]; then
  case "$CASE_ARG" in
    01.all | 02.mixed | 03.restart | 04.full-restart | 05.slow-sst-joiner)
      # shellcheck disable=SC1090,SC1091
      source "${CASES_DIR}/${CASE_ARG}.sh"
      ;;
    *)
      log "Unknown case: $CASE_ARG (use 01.all, 02.mixed, 03.restart, 04.full-restart, 05.slow-sst-joiner)"
      exit 1
      ;;
  esac
else
  ...
  # shellcheck disable=SC1091
  source "${CASES_DIR}/05.slow-sst-joiner.sh"
fi
