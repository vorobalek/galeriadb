...
if [ -n "$CASE_ARG" ]; then
  case "$CASE_ARG" in
    01.all-required | 02.missing-peers | 03.missing-root-password | 04.missing-bootstrap-candidate | 05.healthcheck-user | 06.graceful-shutdown | 07.healthcheck-docker | 08.sst-readiness-wait)
      run_case "$CASE_ARG"
      ;;
    *)
      log "Unknown case: $CASE_ARG. Use 01.all-required | 02.missing-peers | 03.missing-root-password | 04.missing-bootstrap-candidate | 05.healthcheck-user | 06.graceful-shutdown | 07.healthcheck-docker | 08.sst-readiness-wait"
      exit 1
      ;;
  esac
else
