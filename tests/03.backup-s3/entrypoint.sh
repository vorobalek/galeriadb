#!/usr/bin/env bash
# S3 backup test: MinIO + Galera, backup/clone to S3. Runs cases from cases/ (01.backup-to-s3, 02.fail-without-s3-config, 03.retention-deletes-old, 04.cron-backup, 05.clone-on-empty, 06.cluster-restore).
# Usage: ./tests/03.backup-s3/entrypoint.sh [IMAGE] [CASE]
#   CASE: 01.backup-to-s3 | 02.fail-without-s3-config | 03.retention-deletes-old | 04.cron-backup | 05.clone-on-empty | 06.cluster-restore (default: run all in order)
